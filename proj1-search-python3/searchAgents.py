...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        cornerBools = ()
        # Check if we started in a corner.
        index = -1
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0,4):
        if cornerBools[i]:
            continue # visited corner already
        cornerX, cornerY = corners[i]
        currDistance = getManhattanDistance(x, y, cornerX, cornerY)
        if currDistance == 0:
            return 0 # Ensure we have do not have a heuristic > 0 on goal
        maxDistance = max(maxDistance, currDistance) # Ensure we always get the max.
...
def foodHeuristic(state, problem):
    ...
    for i in range(0,len(foodPositions)):
        posX, posY = foodPositions[i]
        currDistance = getManhattanDistance(x, y, posX, posY)
        if currDistance == 0:
            return 0 # Ensure we have do not have a heuristic > 0 on goal
        maxDistance = max(maxDistance, currDistance) # Ensure we always get the max.
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        closestDot = foodList[0]
        closestDistance = mazeDistance(startPosition, closestDot, gameState)
        for dot in foodList:
            distance = mazeDistance(startPosition, dot, gameState)
            if distance < closestDistance:
                closestDistance = distance
                closestDot = dot

        # found closest dot, return path
